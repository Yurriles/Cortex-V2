    ...
    switch (sub) {
      case "blacklist-add":
        ...
        if (!lockdata) {
          ...
          if (lockdata.Channel.includes(blacklistchannel.id))
            return await interaction.reply({
              content: `${client.emoji.error} | That channel is **already** blacklisted!`,
              flags: MessageFlags.Ephemeral,
            });
          else {
      ...
      case "blacklist-remove":
        ...
        if (!lockdataremove) {
          await interaction.reply({
            content: `${client.emoji.cross} | The **blacklist** is **empty**, cannot remove **nothing**..`,
            flags: MessageFlags.Ephemeral,
          });
        } else {
          if (!lockdataremove.Channel.includes(blacklistchannelremove.id))
            return await interaction.reply({
              content: `${client.emoji.error} | That channel is **not** blacklisted!`,
              flags: MessageFlags.Ephemeral,
            });
          else {
            await lockdownschema.updateOne(
      ...
      case "commit":
        ...
        if (!lockdowndata) {
          ...
          const lockmessage = await interaction.reply({
            content: `${client.emoji.cross} | No **lockdown blacklist** detected, are you **SURE** you want to lock all channels? This will cause your permissions to be **overwritten**!`,
            components: [confirm],
            flags: MessageFlags.Ephemeral,
          });
      ...
      case "unlock":
        ...
        if (!unlockdata) {
          ...
          const lockmessage = await interaction.reply({
            content: `${client.emoji.error} | No **lockdown blacklist** detected, are you **SURE** you want to unlock all channels? This will cause your permissions to be **overwritten**!`,
            components: [confirm],
            flags: MessageFlags.Ephemeral,
          });
          const collector = lockmessage.createMessageComponentCollector();
