    ...
    async execute(interaction, client) {
        ...
        switch (interaction.options.getSubcommand()) {
            case'set':
                ...
                interaction.reply({ content: `Slowmode of ${channel} has been set to ${time} ${timeformat}`, flags: MessageFlags.Ephemeral });
                break;
            case 'disable':
                await channel.setRateLimitPerUser(0);
                interaction.reply({ content: `Slowmode of ${channel} has been disabled`, flags: MessageFlags.Ephemeral });
                break;
        }
