const { SlashCommandBuilder, EmbedBuilder, PermissionFlagsBits, PollLayoutType, MessageFlags } = require("discord.js");
const schema = require("../../Schemas/pollDiscord");
...
module.exports = {
    ...
    .addSubcommand(c => c
        ...
        .addStringOption(o => o.setName("poll").setAutocomplete(true).setDescription("Which poll do you want to end? Put the message id."))
    )
    .addSubcommand(c => c
        .setName("view-votes")
        .setDescription("Shows the results of a vote")
        .addStringOption(o => o.setName("poll").setDescription("The poll you want to view the results from? Put the message id.").setAutocomplete(true))
    ),
        ...
        switch (sub) {
            case "create":
                ...
                if (voteOptions.length < 2) {
                    await interaction.reply({ content: "You must provide at least two answers to create a poll.", flags: MessageFlags.Ephemeral });
                    return;
                }
            ...
            case "end":
            const endId = options.getString("poll"); // Check if options is not undefined
            if (!endId) {
                return await interaction.reply({ content: "Invalid poll ID", flags: MessageFlags.Ephemeral });
            }
            endPoll(interaction, endId);
            break;

        case "view-votes":
            const viewId = options.getString("poll"); // Check if options is not undefined
            if (!viewId) {
                return await interaction.reply({ content: "Invalid poll ID", flags: MessageFlags.Ephemeral });
            }
            ...
            default:
                return await interaction.reply({ content: "Command does not exist!", flags: MessageFlags.Ephemeral })
        }
    }
}
